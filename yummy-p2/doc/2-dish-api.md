...
删除原有的 post.js 添加了 dish.js ，描述了一下数据的字段名和数据类型。
...
看看本部分达成的效果。浏览器中，首先访问 localhost:3008/seed-dishes ，可以看到种子数据可以成功添加进 mongodb 数据库，然后访问 /dishes 就可以看到这些数据了。
至此，《添加后端数据》这部分就胜利完成了。
