...
下面的输出 `switched to db express-love-api` 意思是：已经切换到 express-love-api 这个数据库里面了。
...
mongo = {
  ...
  password: '',
  ...
  url:      'mongodb://localhost:27017/express-love-api',
};
```

上面的　`express-love-api` 就是我们要操作的数据库的名字，这个是通过　mongo shell 中，执行

```
show dbs
```

看到的。由于我们的 express-love-api 这个数据库本身没有设置密码，所以上面 username 和 password 两项都改成空字符串就可以了。
...
Connecting to express-love-api...
```
